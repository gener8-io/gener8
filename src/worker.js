...
function get_room_by_name(name) {
  for (var room_type in room_types) {
    if (room_types[room_type].name == name) return room_type;
  }
...
function paint_grid(grid, x, y, width, height, type) {
  ...
  for (var i = hindex; i < grid.heights.length; ++i) {
    ...
    for (var j = windex; j < grid.widths.length; ++j) {
      ...
      if (Math.abs(width - wcursor) < 0.001) break;
    }

    hcursor += grid.heights[i];
    if (Math.abs(height - hcursor) < 0.001) break;
  }
}
...
function grid_value(grid, y, x) {
  if (x >= grid.widths.length || y >= grid.heights.length || x < 0 || y < 0)
    return [-1, -1];
  else return grid.points[y][x];
}
...
function grid_flood_fill(grid, room_type, y, x, neighborhoods) {
  ...
  if (cell[1] == g_evaluation_id || cell[0] == -1) return area;
...
function grid_rect_rect(grid, x, y, w, h) {
  var wcursor = 0.0;
  var hcursor = 0.0;
  var width = 0.0;
  var height = 0.0;
  ...
  return { x: wcursor, y: hcursor, w: width, h: height };
}
  ...
  for (var y = 0; y < grid.heights.length; ++y) {
    for (var x = 0; x < grid.widths.length; ++x) {
      if (grid.points[y][x][0] == room_type) {
        ...
        var rect_real = grid_rect_rect(grid, rect.x, rect.y, rect.w, rect.h);
...
function room_template(rect, room_type) {
  var fit = [];
  if (!templates[room_types[room_type].name]) return fit;
...
function grid_point_evaluate(grid, x, y, room_type, sln, debug) {
  ...
  if (debug) console.log("templates", room_templates);
...
function room_evaluate(grid, room_type, sln, debug) {
  ...
  if (count != 1) {
    ...
    else if (debug) console.log("missing", room_types[room_type].name);
    score -= 1000.0;
  }
  return score;
}
...
function grid_flood_fill_points(grid, room_type, y, x, points) {
  var cell = grid_value(grid, y, x);
  if (cell[1] == g_evaluation_id || cell[0] != room_type) return;
  cell[1] = g_evaluation_id;
...
function grid_point_coords(grid, x, y, room_type) {
  ...
  if (
    Math.abs(coords[coords.length - 1].x - coords[0].x) < 0.001 &&
  ...
  for (var y = 0; y < grid.heights.length; ++y) {
    for (var x = 0; x < grid.widths.length; ++x) {
      ...
      if (cell[0] == room_type && cell[1] != g_evaluation_id) {
        count++;
        if (count == 1) {
          feature.coords = grid_point_coords(grid, x, y, room_type);
        }
        if (count > 1) break;
      }
    }
...
function grid_features(grid) {
  ...
  for (var room_type in room_types) {
    ...
    if (feature.coords.length > 0) features.push(feature);
  }
  return features;
}
...
export const calculateUnitPlan = (constraints) => {
  ...
  intervalId = setInterval(function () {
    var improved = false;
    for (var i = 0; i < 80; ++i) {
      if (population_update(slns)) improved = true;
    }
    if (!improved) {
